...
void Recognize::start(ncnn::Mat &ncnn_img, float *&feature) {
    RecogNet(ncnn_img);
    for (int i = 0; i < feature_out.size(); i++) {
        feature[i] = feature_out[i];
    }
}
...
void
Recognize::warpAffineMatrix(ncnn::Mat src, ncnn::Mat &dst, float *M, int dst_w, int dst_h) {
    int src_w = src.w;
    int src_h = src.h;
